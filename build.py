...
# Base: full catalog = 24 Lab sections × 36 = 864; next bottleneck = Física at 865
sensitivity = []
for k in range(9):
    lab_cap_k  = (24 + k) * 36
    other_cap  = 865  # Física (next bottleneck after Lab in full catalog)
...
<main class="main">
  ...
  <section class="section animate-in" id="cap-uea">
    ...
    <p class="section-desc">
      ...
      teóricas del catálogo tienen capacidad superior (la más baja, Introducción a la Física, llega
      a 865). La gráfica muestra la ocupación de los grupos del ciclo 26-P frente a su cupo
      disponible. Con 674 inscritos, quedan <strong>190 plazas libres</strong> en el Laboratorio.
    </p>
    <div class="read-callout">
    ...
    <div class="chart-guide-grid animate-in delay-1">
      ...
      <div class="howto-card">
        ...
        <div class="howto-step"><div class="howto-icon">4</div>
          <div class="howto-text"><strong>Techo verificado:</strong> la capacidad de 864 fue confirmada evaluando casi dos millones de combinaciones de grupos posibles. El resultado es el mismo en todos los escenarios.</div></div>
      </div>
    </div>
  </section>
  ...
  <section class="section animate-in" id="cap-sens">
    ...
    <p class="section-desc">
      Agregar un grupo de Laboratorio de Reacciones Químicas (36 lugares) desplaza el
      cuello de botella hacia las UEAs de teoría (765). A partir de ese punto, añadir más
      grupos de laboratorio no incrementa la capacidad máxima: para seguir creciendo se
      requieren también grupos adicionales de teoría (45 lugares c/u).
    </p>
    <div class="read-callout">
      ...
      línea horizontal punteada marca el tope de las UEAs de teoría (765). Una vez que el Lab
      supera esa línea, agregar más grupos de lab no ayuda.
    </div>
    <div class="chart-guide-grid animate-in delay-1">
      <div class="chart-card">
        <div class="chart-card-header">
      ...
      <div class="recuadro" style="margin:0;align-self:start">
        <div class="recuadro-title">Proyección de inversión</div>
        <p>La capacidad actual (864) ya absorbe el padrón 26-P sin inversión adicional. El costo marginal de los primeros <strong>190 nuevos estudiantes</strong> (hasta 864) es cero: la infraestructura está instalada y los grupos disponibles.</p>
        <p>Para crecer de 865 a 900 se necesita abrir <strong>1 sección de Lab</strong> en horario de 13:00 (lunes o viernes — sin interferencia con ningún otro grupo). Cada sección agrega 36 plazas.</p>
        <p>Para superar 900 se requieren grupos adicionales de Lab <em>y</em> de Introducción a la Física simultáneamente. Más allá de ese umbral, la expansión necesita coordinar dos UEAs en paralelo.</p>
      </div>
    </div>
  </section>
...
(function(){
  ...
  renderChart('chart-sens',
    () => {
      ...
      const sColors = smaxCap.map(c => c < 865 ? '#C82D23' : '#2E7D32');
      return [
        {
    ...
    () => ({
      ...
      yaxis:{title:'Capacidad máxima',titlefont:{size:10},tickfont:FONT,gridcolor:'#E4E2DF',
             range:[840,940]},
      shapes:[{type:'line',x0:-.5,x1:8.5,y0:865,y1:865,
               line:{color:'#00838F',width:1.5,dash:'dash'}}],
      annotations:[{x:4,y:868,text:'Techo de Intro. a la Física (865)',
